...
// in a typical Q4_K_M GGUF); Q5_K followed as a post-MVP extension; a later
// pass added the legacy 32-element-block formats (Q4_0/1, Q5_0/1, Q8_0/1) and
// the remaining 256-element K-quant formats (Q2_K, Q3_K, Q8_K). This pass
// adds the last 8 formats, the IQ ("i-quant") family (IQ2_XXS/XS/S,
// IQ3_XXS/S, IQ1_S/M, IQ4_XS), closing the on-device dequant set entirely --
// every GGML block-quantized format now dequantizes on-GPU, matching
// llama.cpp's CUDA backend, which never materializes a host f32 copy of
// quantized weights either. Unlike the other formats above, these are
// non-uniform/codebook quant types: each block's raw bits index into a fixed
// lookup table of representative values rather than a uniform integer range,
// so the tables in `dequant_iq_tables.cuh` (mechanically generated by
// `scripts/gen_iq_tables.py` from the already-verified
// `src/dequant_iq_tables.rs`, not hand-transcribed a second time) are loaded
// into `__constant__` memory and indexed per-block below.
//
// Each kernel is a line-for-line port of its host counterpart in
// src/dequant.rs / src/dequant_iq.rs (`dequantize_block_q4_k`/
// `dequantize_block_iq2_xxs`/etc., themselves line-for-line ports of upstream
// ggml-quants.c) -- variable names
// (`d`, `dmin`, `sc`, `m`, `ql`, `qh`, `is`, `shift`, `signs`, `grid`, ...)
// are kept identical on purpose so the CUDA and Rust/C versions can be
// diffed by eye. One CUDA thread handles one whole block (correctness-first,
// matching this project's existing `gemv_kernel`/`rmsnorm_kernel` style --
// no warp-level tricks), parallelized across the (typically tens of
// thousands of) blocks in a weight tensor.

#include <cuda_fp16.h>
#include <cstring>

#include "dequant_iq_tables.cuh"
...
// Helpers shared by the IQ-family kernels below (ports of the matching
// free functions in src/dequant_iq.rs -- same names, same semantics).

__device__ __forceinline__ unsigned short le_u16_at(const unsigned char* b, unsigned int off) {
    return (unsigned short)b[off] | ((unsigned short)b[off + 1] << 8);
}

__device__ __forceinline__ unsigned int le_u32_at(const unsigned char* b, unsigned int off) {
    return (unsigned int)b[off] | ((unsigned int)b[off + 1] << 8) | ((unsigned int)b[off + 2] << 16) | ((unsigned int)b[off + 3] << 24);
}
...
// Byte `j` of a packed 64-bit codebook entry (`(const uint8_t *)&grid[j]`).
__device__ __forceinline__ unsigned char grid_u64_byte(unsigned long long entry, unsigned int j) {
    return (unsigned char)(entry >> (8 * j));
}

// Byte `j` of a packed 64-bit entry interpreted as `int8_t` (iq1s/iq1m grid).
__device__ __forceinline__ signed char grid_u64_i8(unsigned long long entry, unsigned int j) {
    return (signed char)(unsigned char)(entry >> (8 * j));
}

// Byte `j` of a packed 32-bit codebook entry (`(const uint8_t *)&grid[j]`).
__device__ __forceinline__ unsigned char grid_u32_byte(unsigned int entry, unsigned int j) {
    return (unsigned char)(entry >> (8 * j));
}

__device__ __forceinline__ float sign_of(bool cond) {
...
extern "C" __global__ void dequantize_iq2xxs_kernel(
    ...
    unsigned int y_off = 0;
    for (unsigned int ib32 = 0; ib32 < 256 / 32; ib32++) {
        // `x[i].qs` is `uint16_t[32]`, so the reference's `qs + 4*ib32`
        // pointer arithmetic advances 8 bytes per group, not 4.
        unsigned int aux32_0 = le_u32_at(qs, 8 * ib32);
        unsigned int aux32_1 = le_u32_at(qs, 8 * ib32 + 4);
        float db = d * (0.5f + (float)(aux32_1 >> 28)) * 0.25f;
        unsigned char aux8[4];
        aux8[0] = (unsigned char)(aux32_0 & 0xFF);
        aux8[1] = (unsigned char)((aux32_0 >> 8) & 0xFF);
        aux8[2] = (unsigned char)((aux32_0 >> 16) & 0xFF);
        aux8[3] = (unsigned char)((aux32_0 >> 24) & 0xFF);
...
extern "C" __global__ void dequantize_iq2xs_kernel(
    ...
    unsigned int y_off = 0;
    for (unsigned int ib32 = 0; ib32 < 256 / 32; ib32++) {
        unsigned char sc = scales[ib32];
        float db0 = d * (0.5f + (float)(sc & 0xf)) * 0.25f;
        float db1 = d * (0.5f + (float)(sc >> 4)) * 0.25f;
        for (unsigned int l = 0; l < 4; l++) {
            unsigned short qv = le_u16_at(qs, 2 * (4 * ib32 + l));
            unsigned long long grid = IQ2XS_GRID[qv & 511];
...
// Port of `dequantize_block_iq2_s` (dequant_iq.rs). Block layout (82 bytes):
...
extern "C" __global__ void dequantize_iq2s_kernel(
    ...
    const unsigned char* signs = qs + 256 / 8;
    ...
    for (unsigned int ib32 = 0; ib32 < 256 / 32; ib32++) {
        ...
        for (unsigned int l = 0; l < 4; l++) {
            float dl = (l / 2 == 0) ? db0 : db1;
            unsigned int idx = (unsigned int)qs[4 * ib32 + l] | ((((unsigned int)qh[ib32]) << (8 - 2 * l)) & 0x300);
...
extern "C" __global__ void dequantize_iq3xxs_kernel(
    ...
    float d = le_f16(block);
    const unsigned char* qs = block + 2;
    const unsigned char* scales_and_signs = qs + 256 / 4;

    unsigned int y_off = 0;
    for (unsigned int ib32 = 0; ib32 < 256 / 32; ib32++) {
        unsigned int aux32 = le_u32_at(scales_and_signs, 4 * ib32);
...
extern "C" __global__ void dequantize_iq3s_kernel(
    ...
    unsigned int ib32 = 0;
    while (ib32 < 256 / 32) {
        unsigned int k = ib32 / 2;
        unsigned char sc = scales[k];
        float db1 = d * (1.0f + 2.0f * (float)(sc & 0xf));
        float db2 = d * (1.0f + 2.0f * (float)(sc >> 4));

        // First half of the pair: `qh[0]`, `qs` window 16*k, `signs` window 8*k, db1.
        unsigned int qh0 = qh[2 * k];
        for (unsigned int l = 0; l < 4; l++) {
            unsigned int idx1 = (unsigned int)qs[16 * k + 2 * l] | ((qh0 << (8 - 2 * l)) & 256);
            unsigned int idx2 = (unsigned int)qs[16 * k + 2 * l + 1] | ((qh0 << (7 - 2 * l)) & 256);
        ...
        unsigned int qh1 = qh[2 * k + 1];
        for (unsigned int l = 0; l < 4; l++) {
            unsigned int idx1 = (unsigned int)qs[16 * k + 8 + 2 * l] | ((qh1 << (8 - 2 * l)) & 256);
            unsigned int idx2 = (unsigned int)qs[16 * k + 8 + 2 * l + 1] | ((qh1 << (7 - 2 * l)) & 256);
            unsigned int grid1 = IQ3S_GRID[idx1];
            unsigned int grid2 = IQ3S_GRID[idx2];
...
extern "C" __global__ void dequantize_iq1s_kernel(
    ...
    unsigned int y_off = 0;
    for (unsigned int ib = 0; ib < 256 / 32; ib++) {
        unsigned short qh_ib = le_u16_at(qh, 2 * ib);
        float dl = d * (2.0f * (float)((qh_ib >> 12) & 7) + 1.0f);
        float delta = (qh_ib & 0x8000) ? -IQ1S_DELTA : IQ1S_DELTA;
        for (unsigned int l = 0; l < 4; l++) {
...
extern "C" __global__ void dequantize_iq1m_kernel(
    ...
    const unsigned char* sc_bytes = block + 48;

    unsigned short sc0 = le_u16_at(sc_bytes, 0);
    unsigned short sc1 = le_u16_at(sc_bytes, 2);
    unsigned short sc2 = le_u16_at(sc_bytes, 4);
    unsigned short sc3 = le_u16_at(sc_bytes, 6);
    unsigned short scale_u16 = (unsigned short)((sc0 >> 12) | ((sc1 >> 8) & 0x00f0) | ((sc2 >> 4) & 0x0f00) | (sc3 & 0xf000));
    float d = f16_bits_to_f32(scale_u16);
    unsigned short sc[4] = {sc0, sc1, sc2, sc3};

    unsigned int y_off = 0;
    for (unsigned int ib = 0; ib < 256 / 32; ib++) {
        unsigned int shift = 6 * (ib % 2);
        float dl1 = d * (2.0f * (float)((sc[ib / 2] >> shift) & 7) + 1.0f);
        float dl2 = d * (2.0f * (float)((sc[ib / 2] >> (shift + 3)) & 7) + 1.0f);

        unsigned int qs_off = 4 * ib;
        unsigned int qh_off = 2 * ib;
        unsigned int q0 = qh[qh_off];
        unsigned int q1 = qh[qh_off + 1];
        unsigned int idx[4];
        idx[0] = (unsigned int)qs[qs_off] | ((q0 << 8) & 0x700);
        idx[1] = (unsigned int)qs[qs_off + 1] | ((q0 << 4) & 0x700);
        idx[2] = (unsigned int)qs[qs_off + 2] | ((q1 << 8) & 0x700);
        idx[3] = (unsigned int)qs[qs_off + 3] | ((q1 << 4) & 0x700);
        float delta[4];
        delta[0] = (qh[qh_off] & 0x08) ? -IQ1S_DELTA : IQ1S_DELTA;
        delta[1] = (qh[qh_off] & 0x80) ? -IQ1S_DELTA : IQ1S_DELTA;
        delta[2] = (qh[qh_off + 1] & 0x08) ? -IQ1S_DELTA : IQ1S_DELTA;
        delta[3] = (qh[qh_off + 1] & 0x80) ? -IQ1S_DELTA : IQ1S_DELTA;
...
extern "C" __global__ void dequantize_iq4xs_kernel(
    ...
    unsigned int y_off = 0;
    for (unsigned int ib = 0; ib < 256 / 32; ib++) {
        int ls = (int)((unsigned int)((scales_l[ib / 2] >> (4 * (ib % 2))) & 0xf)
