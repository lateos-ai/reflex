...
// over row-major (num_tokens, num_heads, head_dim) `t`. Only the first
// rotary_dim elements of each head are rotated; any remaining head_dim -
// rotary_dim elements are left untouched. For i in [0, rotary_dim/2), pairs
// (x1, x2) = (t[i], t[i + rotary_dim/2]) rotate by theta_i = position /
// base^(2i/rotary_dim).
extern "C" __global__ void rope_kernel(
    ...
    unsigned long long total_pairs = (unsigned long long)num_tokens * num_heads * half_rotary;
    ...
    for (unsigned long long idx = global_tid; idx < total_pairs; idx += (unsigned long long)gridDim.x * blockDim.x) {
        ...
        unsigned long long rem = idx / half_rotary;
        unsigned int head = rem % num_heads;
        unsigned int token = rem / num_heads;
        ...
        unsigned long long base_idx = ((unsigned long long)token * num_heads + head) * head_dim;
