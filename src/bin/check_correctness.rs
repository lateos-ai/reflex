...
fn parse_reference(path: &str) -> Result<Reference, String> {
    let content = std::fs::read_to_string(path).map_err(|e| format!("failed to read reference file {path:?}: {e}"))?;
    let mut lines = content.lines();
    let ids_line = lines.next().ok_or_else(|| format!("reference file {path:?} is empty"))?;
    let token_ids: Vec<u32> = ids_line
        .split(',')
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(|s| s.parse::<u32>().map_err(|e| format!("reference file {path:?}: invalid token id {s:?}: {e}")))
        .collect::<Result<_, _>>()?;
    if token_ids.is_empty() {
        return Err(format!("reference file {path:?}: line 1 must list at least one comma-separated token id"));
    }
    let logit_checksum = match lines.next() {
        Some(line) if !line.trim().is_empty() => Some(
            line.trim()
                .parse::<f64>()
                .map_err(|e| format!("reference file {path:?}: invalid logit_checksum {line:?}: {e}"))?,
        ),
        _ => None,
    };
    Ok(Reference { token_ids, logit_checksum })
}
...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--max-tokens" => {
                let raw = args.next().unwrap_or_else(|| exit_usage_error("--max-tokens requires a number"));
                max_tokens = raw.parse().unwrap_or_else(|_| exit_usage_error(&format!("--max-tokens must be a positive integer, got {raw:?}")));
            }
            "--reference" => reference_path = Some(args.next().unwrap_or_else(|| exit_usage_error("--reference requires a file path"))),
            "--tolerance" => {
                let raw = args.next().unwrap_or_else(|| exit_usage_error("--tolerance requires a number"));
                tolerance = raw.parse().unwrap_or_else(|_| exit_usage_error(&format!("--tolerance must be a number, got {raw:?}")));
            }
    ...
    let file = GgufFile::open(&gguf_path).unwrap_or_else(|e| exit_usage_error(&format!("failed to open {gguf_path}: {e}")));
    let device = diagnostics::init_device_with_diagnostics(0).unwrap_or_else(|e| exit_usage_error(&e));
    if let Ok(diag) = diagnostics::probe(&device) {
        eprintln!("{diag}");
    }
    let model = Model::load(device, &file).unwrap_or_else(|e| exit_usage_error(&format!("failed to load model: {e}")));

    let mut first_logits: Vec<f32> = Vec::new();
    let (token_ids, _text) = model
        .generate(&prompt, max_tokens, None, |logits| first_logits = logits.to_vec())
    ...
    let top1_logit = first_logits.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
