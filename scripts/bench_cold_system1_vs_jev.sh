...
# comparison of coldstart-infer's System1 candidate-scoring path against
...
#
#   - coldstart-infer: MEASURED here, locally, on this GPU — a real cold
#     process launch through System1's single-pass candidate scoring
#     (`Model::system1_evaluate` / the `system1_coldstart` binary),
...
#     coldstart-infer's is a pure local process launch with zero network
...
#     decision model; coldstart-infer's System1 path is a generic
...
#   (expects a release build of `system1_coldstart` at target/release/)
...
bin_path="$repo_root/target/release/system1_coldstart"

if [[ ! -x "$bin_path" ]]; then
  echo "error: $bin_path not found — build it first: cargo build --release --bin system1_coldstart" >&2
  exit 1
fi

"$script_dir/bench_cold_common.sh" system1 "$n_runs" -- \
  "$bin_path" "$gguf_path" "$prompt" --candidate " True" --candidate " False"
